...
func start_minigame(fish_data: Dictionary) -> void:
	...
	print("[FishingMiniGame] Started minigame with fish: " + str(fish_data.get("name", "Unknown")))
...
func _calculate_bite_time() -> float:
	...
	## 鱼饵缩短系数（如果有鱼饵数据）
	var bait_multiplier = _fish_data.get("bait_multiplier", 0.9)

	return base_time * skill_multiplier * bait_multiplier
...
func _start_bite_phase() -> void:
	...
	print("[FishingMiniGame] Bite phase started, waiting for sink > 70%")
...
func _update_bite(delta: float) -> void:
	...
	if _bobber_state == BobberState.SINKING:
		...
		## 检查是否超过70%阈值，触发时机窗口
		if not _is_timing_window_active and _bobber_sink_amount >= SINK_TRIGGER_THRESHOLD:
			## 触发时机窗口
			_is_timing_window_active = true
			var timing_window = BASE_TIMING_WINDOW + (_fish_difficulty * TIMING_WINDOW_PER_DIFFICULTY)
			timing_window = minf(timing_window, MAX_TIMING_WINDOW)
			_timing_window_timer = timing_window
			print("[FishingMiniGame] Timing window started! duration=" + str(timing_window) + "s")
...
func _update_fish_position(delta: float) -> void:
	## 基础速度计算
	var player_speed = PLAYER_BASE_SPEED * (1.0 + _skill_level * 0.05) * _rod_bonus
...
func _get_zone_type(position: float) -> ZoneType:
	if position >= SAFE_ZONE_START and position <= SAFE_ZONE_END:
		return ZoneType.SAFE
	elif position >= SAFE_ZONE_START - TRANSITION_ZONE_SIZE and position < SAFE_ZONE_START:
		return ZoneType.TRANSITION
	elif position > SAFE_ZONE_END and position <= SAFE_ZONE_END + TRANSITION_ZONE_SIZE:
		return ZoneType.TRANSITION
	else:
