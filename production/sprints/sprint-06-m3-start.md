# Sprint 6 -- 2026-04-20 to 2026-04-21

## Sprint Goal

**完成 M3 里程碑开局：商店系统（购买/出售/动物商店）和烹饪系统（食谱/烹饪锅）核心逻辑上线，同步推进本地化和风险关闭。**

**结果**: ✅ 目标达成 (Sprint 6 ended 2026-04-21)
...
| ID | 任务 | 负责人 | 预估 | 状态 | 备注 |
|----|------|--------|------|------|------|
| S6-T1 | 商店系统基础架构 | Dev | 2天 | ✅ 完成 | `ShopSystem.gd` Autoload、购买/出售核心逻辑、营业时间检查 |
| S6-T2 | 商店购买UI | Dev | 1天 | ✅ 完成 | 商品列表显示、购买按钮、余额扣款、背包添加 |
| S6-T3 | 商店出售UI | Dev | 1天 | ✅ 完成 | 背包物品选择、出售价格计算、金币增加 |
| S6-T4 | 动物商店 | Dev | 1天 | ✅ 完成 | 从商店购买动物、放入畜牧系统 |

### Must Have (P0) — 烹饪系统核心

| ID | 任务 | 负责人 | 预估 | 状态 | 备注 |
|----|------|--------|------|------|------|
| S6-T5 | 烹饪系统基础架构 | Dev | 2天 | ✅ 完成 | `CookingSystem.gd` Autoload、配方加载、食谱匹配、产出计算 |
| S6-T6 | 烹饪核心逻辑 | Dev | 1天 | ✅ 完成 | 投入食材、烹饪计时、产出物品、品质继承 |
| S6-T7 | 烹饪UI | Dev | 2天 | ✅ 完成 | 配方选择、食材显示、烹饪锅状态、产出提示 |

### Should Have (P1) — 本地化 + 测试

| ID | 任务 | 负责人 | 预估 | 状态 | 备注 |
|----|------|--------|------|------|------|
| S6-T8 | 本地化工作流执行 | Dev | 1天 | ✅ 完成 | 提取硬编码字符串、生成翻译表、替换tr()调用 |
| S6-T9 | FishPondSystem 单元测试 | Dev | 1天 | ✅ 完成 | add/remove/collect/daily_update 路径覆盖 |
| S6-T10 | FishCompendiumSystem 单元测试 | Dev | 1天 | ✅ 完成 | record/progress 路径覆盖 |

### Nice to Have (P2) — 技术债

| ID | 任务 | 负责人 | 预估 | 状态 | 备注 |
|----|------|--------|------|------|------|
| S6-T11 | game_manager TODO清理 | Dev | 1天 | ⏳ 延期 | UI需单独设计存档菜单 |
| S6-T12 | Bug严重性分级文档 | Dev | 0.5天 | ⏳ 延期 | 优先级低于功能开发 |
